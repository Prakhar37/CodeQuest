...
import { updateUserStats } from './user.js';
import dotenv from "dotenv";
import Docker from "dockerode"; // Add Dockerode
dotenv.config();

const docker = new Docker(); // Initialize Dockerode
const __dirname = dirname(fileURLToPath(import.meta.url));
...
// Function to execute code inside Docker containers using Dockerode
const executeCode = async (filePath, language, inputPath, timeLimit = 5, memoryLimit = "256m") => {
    let containerName, command;

    switch (language) {
        case "cpp":
            containerName = "cpp-docker";
            command = "g++ /code/code.cpp -o /code/code.out && /code/code.out < /input.txt";
            break;
        case "java":
            containerName = "java-docker";
            command = "javac /code/Main.java && java -cp /code Main < /input.txt";
            break;
        case "python":
            containerName = "python-docker";
            command = "python /code/code.py < /code/input.txt";
            break;
        case "javascript":
            containerName = "node-docker";
            command = "node /code/code.js < /code/input.txt";
            break;
        default:
    ...
    try {
        const container = await docker.createContainer({
            ...
            Tty: false,
            Binds: [
                `${filePath}:/code/${path.basename(filePath)}`,
                `${inputPath}:/input.txt`,
            ],
            HostConfig: {
                Memory: memoryLimit === "unlimited" ? 0 : parseInt(memoryLimit),
                NanoCpus: 1000000000, // 1 CPU
                CpuQuota: timeLimit * 100000, // Time limit
            }
        });
...
router.post("/run", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            res.status(500).json({ success: false, message: "Error executing code", error });
        }
    } catch (error) {
        res.status(500).json({ success: false, message: "Failed to execute code", error });
    } finally {
        cleanupFiles();
    }
});
